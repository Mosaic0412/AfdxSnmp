...
#include <library/snmpUDPIPv4BaseDomain.h>
...
int netsnmp_udp_sendto(int fd, struct in_addr *srcip, int if_index, struct sockaddr *remote,
	void *data, int len, netsnmp_address_list * address)
{
    /** udpiv4 just calls udpbase. should we skip directly to there? */
    return netsnmp_udpipv4_sendto(fd, srcip, if_index, remote, data, len,address);
}
