...
#include <library/snmp.h>      /* for xdump & {build,parse}_var_op */
...
netsnmp_pdu *pdu, snmp_callback callback, void *cb_data)
{
	...
	address = pdu->address;
	result = netsnmp_transport_send(transport, packet, length, &(pdu->transport_data), &(pdu->transport_data_length), address);
...
int incr_retries)
{
	...
	netsnmp_address_list * address;
	address = rp->pdu->address;
	result = netsnmp_transport_send(transport, packet, length, &(rp->pdu->transport_data), &(rp->pdu->transport_data_length), address);

	/*############################################*/
	/*############################################*/
	//result = l_TransmitAFDXSetup(1, 0, rp->pdu, length);
	/*############################################*/
	/*############################################*/
